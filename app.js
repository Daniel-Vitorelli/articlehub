...
(function () {
  ...
  function statusLabel(s) {
    return { pending: 'Pendente', 'in-progress': 'Em Produção', review: 'Em Revisão', done: 'Concluído', published: 'Publicado', revisado: 'Revisado' }[s] || s;
  }
  ...
  function roleLabel(r) {
    return { admin: 'Administrador', gestor: 'Gestor de Tráfego', revisor: 'Revisor', redator: 'Redator' }[r] || r;
  }
  ...
  function fieldLabel(field) {
    return {
      ...
      blog: 'Blog', domain_id: 'Blog',
      writer: 'Redator', writer_id: 'Redator',
      priority: 'Prioridade',
  ...
  async function loadAll() {
    try {
      const [reqData, delReqData, domData, langData, nicheData, notifData, usersData] = await Promise.all([
        apiGet('requests.php'),
  ...
  //  PERMISSIONS
  // ============================================
  function is(role) { return currentUser && currentUser.role === role; }
  function canCreate() { return !!currentUser; }
  function canDelete() { return is('admin'); }
  function canManageUsers() { return is('admin'); }
  function canManageDomains() { return is('admin'); }
  ...
  function canEdit(r) {
    ...
    if ((is('gestor') || is('redator')) && Number(r.requested_by_id) === Number(currentUser.id)) return true;
    return false;
  }

  function canManagePendency(r) {
    if (is('admin') || is('revisor') || is('redator')) return true;
    if (is('gestor') && Number(r.requested_by_id) === Number(currentUser.id)) return true;
    return false;
  }
  ...
  function applyRoleUI() {
    ...
    $$('.admin-only').forEach(el => { el.style.display = is('admin') ? '' : 'none'; });
    $$('.gestor-admin-only').forEach(el => { el.style.display = canCreate() ? '' : 'none'; });
    // 'gestor-col' logic removed as Solicitante is now visible to all
    $$('.revisor-admin-only').forEach(el => { el.style.display = (is('admin') || is('revisor')) ? '' : 'none'; });
  }
  ...
  function renderDashboard() {
    ...
    const sorted = [...visible].sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).slice(0, 5);
    ...
    tbody.querySelectorAll('[data-detail-id]').forEach(row => {
      row.addEventListener('click', () => openDetail(Number(row.dataset.detailId)));
    });
  ...
  function populateRequestFilters() {
    ...
    blogSelect.innerHTML = '<option value="">Todos Blogs</option>' +
      domains
        .filter(d => d.active)
        .sort((a, b) => (a.blog_name || '').localeCompare(b.blog_name || ''))
        .map(d => `<option value="${d.id}">${escapeHtml(d.blog_name || '—')}</option>`)
        .join('');
    blogSelect.value = currentBlog;

    // Redatores: do banco (users com role redator)
    const writerSelect = $('#filterWriter');
    writerSelect.innerHTML = '<option value="">Todos Redatores</option>' +
      users
    ...
    requesterSelect.innerHTML = '<option value="">Todos Solicitantes</option>' +
      users
        .filter(u => u.active)
        .sort((a, b) => (a.name || '').localeCompare(b.name || ''))
        .map(u => `<option value="${u.id}">${escapeHtml(u.name || '—')}</option>`)
  ...
  function renderRequests() {
    ...
    if (priorityFilter) visible = visible.filter(r => r.priority === priorityFilter);
    if (blogFilter) visible = visible.filter(r => String(r.domain_id) === blogFilter);
    if (writerFilter) visible = visible.filter(r => String(r.writer_id) === writerFilter);
    if (requesterFilter) visible = visible.filter(r => String(r.requested_by_id) === requesterFilter);
    if (search) {
      visible = visible.filter(r => {
        const h = `${r.keyword} ${r.blog_name || ''} ${r.writer_name || ''} ${r.requester_name || ''}`.toLowerCase();
        return h.includes(search);
      });
    }

    const statusOrder = { pending: 0, 'in-progress': 1, review: 2, done: 3, published: 4, revisado: 5 };
    visible.sort((a, b) => {
    ...
    tbody.innerHTML = visible.map(r => {
      ...
      const canSoftDelete = r.status === 'pending' &&
      ...
      const pendencyIconColor = Number(r.unresolved_pendencies_count) > 0 ? 'color: var(--accent-danger); font-weight: bold; font-size: 1.1em;' : 'color: rgba(255,255,255,0.1); filter: grayscale(1);';
    ...
    $('#tableInfo').textContent = `Mostrando ${visible.length} de ${getVisibleRequests().length} solicitações`;
  ...
  function renderTrash() {
    ...
    tbody.innerHTML = deletedRequests.map(r => {
      ...
      const canRestore = is('admin') || Number(r.requested_by_id) === Number(currentUser.id);
      ...
      const originalStatusText = '<span class="status-badge disabled">Cancelado</span>';
    ...
    $('#trashTableInfo').textContent = `Mostrando ${deletedRequests.length} solicitações canceladas`;
    ...
    tbody.querySelectorAll('[data-hard-delete-id]').forEach(btn => {
      btn.addEventListener('click', (e) => { 
        e.stopPropagation(); 
        if (confirm('Atenção: A exclusão permanente não pode ser desfeita. Deseja excluir definitivamente esta solicitação?')) {
  ...
  function renderPendenciesList() {
    ...
    if (currentPendencies.length === 0) {
      listEl.innerHTML = '<p class="text-muted" style="text-align:center; padding: 20px 0;">Nenhuma pendência encontrada.</p>';
      return;
    }

    listEl.innerHTML = currentPendencies.map(p => {
      const isResolved = p.status === 'resolved';
      const statusClass = isResolved ? 'resolved' : 'unresolved';
      const actionText = isResolved ? 'Reabrir pendência' : 'Marcar como resolvido';
  ...
  window.togglePendencyStatus = async function(id, newStatus) {
    try {
      await apiPost('pendencies.php', { action: 'update_status', id, status: newStatus });
  ...
  $$('#modalPendencies .modal-close, #modalPendencies .btn-secondary').forEach(btn => {
  ...
  $('#btnSubmitPendency').addEventListener('click', async (e) => {
    ...
    if (!desc) { alert('Digite a descrição da pendência.'); return; }
  ...
  function renderDomains() {
    ...
    tbody.querySelectorAll('[data-edit-domain]').forEach(btn => {
      btn.addEventListener('click', () => openEditDomain(Number(btn.dataset.editDomain)));
    });
    tbody.querySelectorAll('[data-delete-domain]').forEach(btn => {
      btn.addEventListener('click', () => deleteDomain(Number(btn.dataset.deleteDomain)));
    });
  }
  ...
  function populateRequestSelects(blogSelectId, writerSelectId, prefix = '') {
    ...
    domains.filter(d => d.active).forEach(d => {
    ...
    const writerList = users.length > 0
    ...
    if (langSelect) {
      ...
      languages.filter(l => l.active).forEach(l => {
    ...
    if (nicheSelect) {
      ...
      niches.filter(n => n.active).forEach(n => {
  ...
  function openNewRequest() {
    ...
    const deadlineInput = $('#formNewRequest').querySelector('[name="deadline"]');
  ...
  function fillEditForm(r) {
    ...
    form.querySelector('[name="content_type"]').value = r.content_type || 'artigo';
  ...
  function openDetail(id) {
    ...
    const statuses = ['pending', 'in-progress', 'review', 'done', 'published', 'revisado'];
    const sLabels = { pending: 'Pendente', 'in-progress': 'Em Produção', review: 'Em Revisão', done: 'Concluído', published: 'Publicado', revisado: 'Revisado' };

    const currentIdx = statuses.indexOf(r.status);
    const canChange = canChangeStatus(r);

    const stepsHtml = statuses.map((s, i) => {
      let cls = s === 'published' ? 'published-step' : (s === 'revisado' ? 'revisado-step' : '');
    ...
    const publishedLinkHtml = r.status === 'published' && r.published_url
    ...
    const wpEditLinkHtml = ['done', 'revisado', 'published'].includes(r.status) && r.wp_edit_url
      ? `<div class="detail-section">
    ...
    const purposeLabel = { conteudo: 'Conteúdo', arbitragem: 'Arbitragem' }[r.purpose] || r.purpose || '—';
    const typeLabel = { artigo: 'Artigo', pagina: 'Página' }[r.content_type] || r.content_type || '—';
    ...
    if (canChange) {
      $('#detailContent').querySelectorAll('.status-step:not(.disabled)').forEach(step => {
    ...
    const items = [...r.history].reverse().map(h => {
      ...
      if (h.action === 'edit' && changes.length > 0) {
        changesHtml = changes.map(c => {
          ...
          const fromVal = c.field === 'status' ? statusLabel(c.from) : (c.from || '—');
          const toVal = c.field === 'status' ? statusLabel(c.to) : (c.to || '—');
  ...
  async function updateRequestStatus(id, newStatus, extraData = {}) {
    try {
      await apiPut('requests.php?action=status', { id, status: newStatus, ...extraData });
      await loadAll();
      refreshCurrentView();
      openDetail(id);
    } catch (err) {
  ...
  async function deleteRequest(id) {
    const r = requests.find(req => req.id == id);
    if (!r) return;
    const canSoftDelete = is('admin') || is('revisor') || is('redator') ||
  ...
  function openPublishModal(reqId) {
    ...
    if (r.status !== 'done') {
      alert('O artigo precisa estar com status "Concluído" antes de ser publicado.');
      return;
    }
  ...
  async function submitDone() {
    ...
    try { new URL(url); } catch {
      ...
      statusText.textContent = 'URL inválida. Insira uma URL completa (ex: https://...) ';
  ...
  async function submitUser() {
    ...
    try {
      if (editId) {
        const data = { id: editId, name: fd.get('name'), email: fd.get('email'), role: fd.get('role') };
  ...
  async function submitDomain() {
    ...
    const colors = ['#7f5af0', '#2cb67d', '#39a0ed', '#f0a500', '#e53170', '#ff6b6b', '#48bfe3', '#f72585'];
  ...
  async function toggleTheme() {
    ...
    // Save to API
    try { await apiPut('preferences.php', { theme: next }); } catch (e) { /* silent */ }
  }
  ...
  function renderLanguages() {
    ...
    tbody.querySelectorAll('[data-edit-lang]').forEach(btn => {
      btn.addEventListener('click', () => openEditLanguage(Number(btn.dataset.editLang)));
    });
    tbody.querySelectorAll('[data-delete-lang]').forEach(btn => {
      btn.addEventListener('click', () => deleteLanguage(Number(btn.dataset.deleteLang)));
    });
  }
  ...
  async function submitLanguage() {
    ...
    const data = { name: fd.get('name'), code: fd.get('code'), active: fd.get('active') === '1' };
    try {
      if (id) {
        data.id = Number(id);
  ...
  function renderNiches() {
    ...
    tbody.querySelectorAll('[data-edit-niche]').forEach(btn => {
      btn.addEventListener('click', () => openEditNiche(Number(btn.dataset.editNiche)));
    });
    tbody.querySelectorAll('[data-delete-niche]').forEach(btn => {
      btn.addEventListener('click', () => deleteNiche(Number(btn.dataset.deleteNiche)));
    });
  }
  ...
  async function markAllNotifsRead() {
    try {
      ...
      notifications.forEach(n => { n.is_read = 1; n.read = true; });
      updateNotifBadge();
      renderNotifDropdown();
    } catch (e) { /* silent */ }
  }
  ...
  function updateMsgBadge() {
    const unread = notifications.filter(n => n.type === 'new_message' && !n.is_read && !n.read).length;
  ...
  async function renderLogs() {
    ...
    try {
      ...
      tbody.innerHTML = logs.map(log => {
        ...
        const userRole = log.user_role ? ` <span class="role-tag ${log.user_role}">${roleLabel(log.user_role)}</span>` : '';
  ...
  async function renderMessages() {
    ...
    try {
      ...
      if (visible.length === 0) {
        listEl.innerHTML = '<div class="empty-state"><div class="empty-icon">📭</div><p>Nenhuma mensagem.</p></div>';
      ...
      $('#msgInfo').textContent = `${visible.length} mensagen${visible.length === 1 ? '' : 's'}`;

      listEl.querySelectorAll('.msg-item').forEach(el => {
        el.addEventListener('click', () => openMsgDetail(Number(el.dataset.msgId)));
      });
    } catch (e) {
      listEl.innerHTML = '<div class="empty-state"><div class="empty-icon">⚠️</div><p>Erro ao carregar mensagens.</p></div>';
    }
  }
  ...
  async function openCompose(toId, subject) {
    ...
    if (allUsers.length === 0) {
      try { allUsers = await apiGet('users.php'); users = allUsers; } catch (e) { allUsers = []; }
    }

    const select = $('#composeToId');
    select.innerHTML = '<option value="">Selecione...</option>' +
      allUsers.filter(u => u.active && u.id != currentUser.id)
        .map(u => `<option value="${u.id}" ${u.id == toId ? 'selected' : ''}>${escapeHtml(u.name)} (${roleLabel(u.role)})</option>`)
        .join('');
  ...
  function bindEvents() {
    ...
    $('#btnSubmitRequest').addEventListener('click', (e) => { e.preventDefault(); submitRequest(); });

    // Edit request
    $('#btnSubmitEdit').addEventListener('click', (e) => { e.preventDefault(); submitEditRequest(); });

    // Publish
    $('#btnSubmitPublish').addEventListener('click', (e) => { e.preventDefault(); submitPublish(); });
    // Done (WP Edit URL)
    $('#btnSubmitDone').addEventListener('click', (e) => { e.preventDefault(); submitDone(); });

    // User CRUD
    $('#btnNewUser').addEventListener('click', openNewUser);
    $('#btnSubmitUser').addEventListener('click', (e) => { e.preventDefault(); submitUser(); });

    // Domain CRUD
    $('#btnNewDomain').addEventListener('click', openNewDomain);
    $('#btnSubmitDomain').addEventListener('click', (e) => { e.preventDefault(); submitDomain(); });
    ...
    $('#btnSubmitLanguage').addEventListener('click', (e) => { e.preventDefault(); submitLanguage(); });

    // Niches CRUD
    $('#btnNewNiche').addEventListener('click', openNewNiche);
    $('#btnSubmitNiche').addEventListener('click', (e) => { e.preventDefault(); submitNiche(); });
    ...
    $('#btnSendMessage').addEventListener('click', (e) => { e.preventDefault(); sendMessage(); });
  }
